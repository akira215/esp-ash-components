...
public:
                    ...
                    ) 
    {
        ...
        _attr_list = esp_zb_pressure_meas_cluster_create(&cfg);

        _init(ESP_ZB_ZCL_CLUSTER_ID_PRESSURE_MEASUREMENT, isClient);
    }

    ///@brief Copy constructor
    ZbPressureMeasCluster(const ZbPressureMeasCluster& other)
    {
        esp_zb_pressure_meas_cluster_cfg_t cfg;

        cfg.measured_value  = 
                *((int16_t*)(other.getAttribute((uint16_t)
                    ESP_ZB_ZCL_ATTR_PRESSURE_MEASUREMENT_VALUE_ID)->data_p));
        cfg.min_value = *((int16_t*)(other.getAttribute((uint16_t)
                    ESP_ZB_ZCL_ATTR_PRESSURE_MEASUREMENT_MIN_VALUE_ID)->data_p));
        cfg.max_value = *((int16_t*)(other.getAttribute((uint16_t)
                    ESP_ZB_ZCL_ATTR_PRESSURE_MEASUREMENT_MAX_VALUE_ID)->data_p));   

        _attr_list = esp_zb_pressure_meas_cluster_create(&cfg);

        _copyAttributes(other);

        _init(ESP_ZB_ZCL_CLUSTER_ID_PRESSURE_MEASUREMENT, other.isClient());
    }
    ...
    virtual void addAttribute(uint16_t attr_id, void* value)
    {
        ...
        ESP_ERROR_CHECK(esp_zb_pressure_meas_cluster_add_attr(_attr_list, 
                            attr_id, value));
    ...
    {
        ESP_ERROR_CHECK( esp_zb_cluster_list_add_pressure_meas_cluster(cluster_list, _attr_list, 
