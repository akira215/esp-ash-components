...
public:
    ZbAnalogValueCluster( bool isClient = false,
                    bool out_of_service = ESP_ZB_ZCL_ANALOG_VALUE_OUT_OF_SERVICE_DEFAULT_VALUE,
                    float_t present_value = 0,
                    uint8_t status_flags = ESP_ZB_ZCL_ANALOG_VALUE_STATUS_FLAGS_DEFAULT_VALUE 
                    ) 
    {
        esp_zb_analog_value_cluster_cfg_t cfg;
        cfg.out_of_service  = out_of_service;
        cfg.present_value = present_value;
        cfg.status_flags = status_flags; 

        _attr_list = esp_zb_analog_value_cluster_create(&cfg);

        _init(ESP_ZB_ZCL_CLUSTER_ID_ANALOG_VALUE, isClient);
    }

    ///@brief Copy constructor
    ZbAnalogValueCluster(const ZbAnalogValueCluster& other)
    {
        esp_zb_analog_value_cluster_cfg_t cfg;
        ...
        _init(ESP_ZB_ZCL_CLUSTER_ID_ANALOG_VALUE, other.isClient());
    }

    virtual void addAttribute(uint16_t attr_id, void* value)
    {
        ESP_ERROR_CHECK(esp_zb_analog_value_cluster_add_attr(_attr_list, 
                            attr_id, value));
    }

    virtual void addToList(esp_zb_cluster_list_t* cluster_list)
    {
        ESP_ERROR_CHECK(esp_zb_cluster_list_add_analog_value_cluster(cluster_list, _attr_list, 
